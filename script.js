  ...
  switch (e.key) {
    ...
    case "9": {
      if (this.resultDone) {
        ...
        if (
          this.items.length == 0 ||
          String(this.items[this.itemIndex]).length <= 15
        ) {
          if (this.constEdit) {
            this.inputStr += e.key;
      ...
      if (this.addMode) {
        ...
        if (this.constEdit) {
          if (String(this.items[this.itemIndex]).length <= 15)
    ...
    case "Backspace":
    case "Delete": {
      if (computedStr.textContent.length != 0 && this.inputStr.length != 0) {
        let opStr = "**, +, -, /, *, (, )";
        let bracketExist = false;

        if (!this.resultDone) {
          computedStr.textContent = computedStr.textContent.substring(
            0,
            computedStr.textContent.length - 1
          );

          if (
            this.items[this.itemIndex] == exp ||
            this.items[this.itemIndex] == pi ||
            this.items[this.itemIndex] == "**"
          ) {
            this.inputStr = this.inputStr.substring(
              0,
              this.inputStr.length - this.items[this.itemIndex].length
            );
          } else {
            if (
              this.inputStr[this.inputStr.length - 1] == ")" ||
              this.inputStr[
                this.inputStr.length - this.items[this.itemIndex].length - 2
              ] == "(" ||
              this.inputStr[
                this.inputStr.length - this.items[this.itemIndex].length - 1
              ] == "("
            ) {
              bracketExist = true;
            }
          ...
          if (
            ...
            if (!bracketExist)
              this.items[this.itemIndex] = this.items[this.itemIndex].substring(
                0,
                this.items[this.itemIndex].length - 1
              );
          }

          if (this.items[this.itemIndex] == "" && this.items.length != 1) {
            this.itemIndex--;
            this.items.pop();
          }
          ...
          opStr.split(", ").forEach((item) => {
            if (
              ...
              resultStr.textContent = setRightValue(
                eval(this.items.slice(0, this.items.length - 1).join(""))
              );
            } else if (
              item == this.items[this.itemIndex] &&
              item == this.sign
          ...
          if (
            !bracketExist &&
            opStr.split(", ").indexOf(this.items[this.itemIndex]) == -1
          )
            resultStr.textContent = setRightValue(eval(this.inputStr));
        } else {
          computedStr.textContent = this.inputStr = resultStr.textContent;
          computedStr.textContent = this.inputStr =
            computedStr.textContent.substring(
              0,
              computedStr.textContent.length - 1
            );
          this.items[this.itemIndex] = this.items[this.itemIndex].substring(
    ...
    /***************** Процент от числа ******************/
    case "%": {
      if (this.items.length > 0 && this.currentOperation === null) {
        if (this.resultDone) {
          resultStr.style.cssText = `font-size: 1em;
          transition: .2s ease-in-out`;
        ...
        computedStr.textContent = computedStr.textContent.substring(
          0,
          computedStr.textContent.length - this.items[this.itemIndex].length
        );
        this.inputStr = this.inputStr.substring(
          0,
          this.inputStr.length - this.items[this.itemIndex].length
        );
        this.items[this.itemIndex] = setRightValue(
          String(this.items[this.itemIndex] / 100)
        );
      ...
      if (this.items.length != 0) {
        if (this.resultDone)
          resultStr.style.cssText = `font-size: 1em;
                                                        transition: .2s ease-in-out`;
        if (
          !this.items[this.itemIndex].includes(".") &&
          this.items[this.itemIndex].length > 0 &&
          "*/-+".indexOf(this.items[this.itemIndex]) == -1
        ) {
          this.items[this.itemIndex] += e.key;
          computedStr.textContent += e.key;
          this.inputStr += e.key;
    ...
    case "Shift" && "E": {
      if (
        (this.currentOperation != null || this.items.length == 0) &&
        this.extendMode
      ) {
    ...
    case "Shift" && "N": {
      if (
        ...
        this.items[this.itemIndex] = setRightValue(
          Math.log(this.items[this.itemIndex])
        );
        this.inputStr += this.items[this.itemIndex];
        computedStr.textContent += this.items[this.itemIndex];
        resultStr.textContent = setRightValue(eval(this.inputStr));
    ...
    case "Shift" && "G": {
      if (
        ...
        this.items[this.itemIndex] = setRightValue(
          Math.log10(this.items[this.itemIndex])
        );
        this.inputStr += this.items[this.itemIndex];
        computedStr.textContent += this.items[this.itemIndex];
        resultStr.textContent = setRightValue(eval(this.inputStr));
      ...
      if (
        ...
        if (this.items.length == 0) {
          ...
          if (
            ...
            computedStr.textContent = computedStr.textContent.substring(
              0,
              computedStr.textContent.length -
                String(this.items[this.itemIndex]).length -
                2
            );
            this.inputStr = this.inputStr.substring(
              0,
              this.inputStr.length -
                String(this.items[this.itemIndex]).length -
                2
            );
            ...
            if (this.items.length > 1) {
              ...
              this.inputStr = this.inputStr.substring(
                0,
                this.inputStr.length - String(this.items[this.itemIndex]).length
              );
              this.items[this.itemIndex] = String(-this.items[this.itemIndex]);
              computedStr.textContent += `(${this.items[this.itemIndex]})`;
              this.inputStr += `(${this.items[this.itemIndex]})`;
    ...
    case "Shift" && "D": {
      if (this.items.length > 0 && this.extendMode) {
        if (this.currentOperation === null) {
          if (this.resultDone) {
            resultStr.style.cssText = `font-size: 1.2em;
            transition: .2s ease-in-out`;
    ...
    case "Shift" && "B": {
      if (
        ...
        this.items[this.itemIndex] = setRightValue(
          Math.sqrt(this.items[this.itemIndex])
        );
        this.inputStr += this.items[this.itemIndex];
        computedStr.textContent += this.items[this.itemIndex];
        resultStr.textContent = setRightValue(eval(this.inputStr));
    ...
    case "Shift" && "W": {
      if (
        ...
        this.items[this.itemIndex] = setRightValue(
          factorial(this.items[this.itemIndex])
        );
        this.inputStr += this.items[this.itemIndex];
        computedStr.textContent += this.items[this.itemIndex];
        resultStr.textContent = setRightValue(eval(this.inputStr));
...
calcBtns.forEach((item) => {
  item.addEventListener("click", function () {
    if (Calc.resultDone) {
      ...
      if (
        Calc.items.length == 0 ||
        String(Calc.items[Calc.itemIndex]).length <= 15
      ) {
        if (Calc.constEdit) {
    ...
    if (Calc.addMode) {
      ...
      if (Calc.constEdit) {
        if (String(Calc.items[Calc.itemIndex]).length <= 15)
...
operationBtns.forEach((item) => {
  item.addEventListener("click", function () {
    switch (item.textContent) {
      case "%": {
        if (Calc.items.length > 0 && Calc.currentOperation === null) {
          if (Calc.resultDone)
            resultStr.style.cssText = `font-size: 1em;
                                                          transition: .2s ease-in-out`;
          computedStr.textContent = computedStr.textContent.substring(
            0,
            computedStr.textContent.length - Calc.items[Calc.itemIndex].length
          );
          Calc.inputStr = Calc.inputStr.substring(
            0,
            Calc.inputStr.length - Calc.items[Calc.itemIndex].length
          );
          Calc.items[Calc.itemIndex] = setRightValue(
            String(Calc.items[Calc.itemIndex] / 100)
          );
        ...
        if (Calc.items.length != 0) {
          if (Calc.resultDone)
            resultStr.style.cssText = `font-size: 1em;
                                                          transition: .2s ease-in-out`;
          if (
            !Calc.items[Calc.itemIndex].includes(".") &&
            Calc.items[Calc.itemIndex].length > 0 &&
            "*/-+".indexOf(Calc.items[Calc.itemIndex]) == -1
          ) {
            Calc.items[Calc.itemIndex] += item.textContent;
            computedStr.textContent += item.textContent;
            Calc.inputStr += item.textContent;
...
deleteBtn.addEventListener("click", function () {
  if (
    computedStr.textContent.length != 0 &&
    ...
    if (!Calc.resultDone) {
      ...
      if (
        ...
        Calc.items[Calc.itemIndex] = Calc.items[Calc.itemIndex].substring(
          0,
          Calc.items[Calc.itemIndex].length - 1
        );
      }

      if (Calc.items[Calc.itemIndex] == "" && Calc.items.length != 1) {
        Calc.itemIndex--;
        Calc.items.pop();
      }

      if (Calc.items[Calc.itemIndex] == pi || Calc.items[Calc.itemIndex] == exp)
      ...
      opStr.split(", ").forEach((item) => {
        if (item == Calc.items[Calc.itemIndex]) {
          ...
          resultStr.textContent = setRightValue(
            eval(Calc.items.slice(0, Calc.items.length - 1).join(""))
          );
        }
      });
      ...
      if (
        opStr.split(", ").indexOf(Calc.items[Calc.itemIndex]) != -1 &&
        Calc.items.length == 1
      ) {
        Calc.items.pop();
        Calc.currentOperation = null;
      ...
      if (Calc.items.length == 0 || Calc.items[0] == "") {
...
piBtn.addEventListener("click", function () {
  if (
    (Calc.currentOperation != null || Calc.items.length == 0) &&
    Calc.extendMode
  ) {
    Calc.constEdit = false;
...
radicalBtn.addEventListener("click", function () {
  if (
    ...
    Calc.items[Calc.itemIndex] = setRightValue(
      factorial(Calc.items[Calc.itemIndex])
    );
    Calc.inputStr += Calc.items[Calc.itemIndex];
    computedStr.textContent += Calc.items[Calc.itemIndex];
    resultStr.textContent = setRightValue(eval(Calc.inputStr));
...
rootBtn.addEventListener("click", function () {
  if (
    ...
    Calc.items[Calc.itemIndex] = setRightValue(
      Math.sqrt(Calc.items[Calc.itemIndex])
    );
    Calc.inputStr += Calc.items[Calc.itemIndex];
    computedStr.textContent += Calc.items[Calc.itemIndex];
    resultStr.textContent = setRightValue(eval(Calc.inputStr));
...
degreeBtn.addEventListener("click", function () {
  if (Calc.items.length > 0 && Calc.extendMode) {
    if (Calc.currentOperation === null) {
      if (Calc.resultDone) {
        resultStr.style.cssText = `font-size: 1.2em;
      transition: .2s ease-in-out`;
...
lgBtn.addEventListener("click", function () {
  if (
    ...
    Calc.items[Calc.itemIndex] = setRightValue(
      Math.log10(Calc.items[Calc.itemIndex])
    );
    Calc.inputStr += Calc.items[Calc.itemIndex];
    computedStr.textContent += Calc.items[Calc.itemIndex];
    resultStr.textContent = setRightValue(eval(Calc.inputStr));
...
lnBtn.addEventListener("click", function () {
  if (
    ...
    Calc.items[Calc.itemIndex] = setRightValue(
      Math.log(Calc.items[Calc.itemIndex])
    );
    Calc.inputStr += Calc.items[Calc.itemIndex];
    computedStr.textContent += Calc.items[Calc.itemIndex];
    resultStr.textContent = setRightValue(eval(Calc.inputStr));
...
changeSignBtn.addEventListener("click", function () {
  if (
    ...
    if (Calc.items.length == 0) {
      ...
      if (
        Calc.inputStr[
          Calc.inputStr.length - Calc.items[Calc.itemIndex].length - 2
        ] == "(" &&
        ...
        computedStr.textContent = computedStr.textContent.substring(
          0,
          computedStr.textContent.length -
            String(Calc.items[Calc.itemIndex]).length -
            2
        );
        Calc.inputStr = Calc.inputStr.substring(
          0,
          Calc.inputStr.length - String(Calc.items[Calc.itemIndex]).length - 2
        );
        ...
        if (Calc.items.length > 1) {
          ...
          Calc.inputStr = Calc.inputStr.substring(
            0,
            Calc.inputStr.length - String(Calc.items[Calc.itemIndex]).length
          );
          Calc.items[Calc.itemIndex] = String(-Calc.items[Calc.itemIndex]);
          computedStr.textContent += `(${Calc.items[Calc.itemIndex]})`;
          Calc.inputStr += `(${Calc.items[Calc.itemIndex]})`;
...
toggler.addEventListener("click", function () {
  if (checkbox.checked) {
    ...
    settingsTitle.innerHTML = "Show instruction"
    toggler.style.cssText = `transform: translateX(-1px)`;
  } else {
    checkbox.checked = true;

    settingsTitle.innerHTML = "Hide instruction"
  ...
  if (value != "") {
    if (value.includes(".")) {
      ...
      if (indexOfExp != -1) {
        ...
        strAfterDot = strAfterDot.substring(
          0,
          strAfterDot.length - expressionWithExp.length
        );
      }
      ...
      numArray.split("").forEach((item) => {
        strAfterDot.length > 17
          ? (indexForFix = strAfterDot
              .substring(0, strAfterDot.length - 1)
              .lastIndexOf(item))
          : (indexForFix = strAfterDot.lastIndexOf(item));
        arrayOfIndexes.push(indexForFix);
      });
      if (
        Math.max(...arrayOfIndexes) >= 9 &&
        numArray.indexOf(strAfterDot[strAfterDot.length - 2]) == -1
      ) {
        let rightValue = (+String(value).substring(
          0,
          String(value).length - expressionWithExp.length
        )).toFixed(9);
        ...
        let rightValue = (+String(value).substring(
          0,
          String(value).length - expressionWithExp.length
        )).toFixed(Math.max(...arrayOfIndexes) + 1);
        strAfterDot = rightValue.split(".")[1];
        return strAfterDot[strAfterDot.length - 1] == "0"
          ? setRightValue(rightValue + expressionWithExp)
          : rightValue + expressionWithExp;
...
function checkEmptiness() {
  computedStr.textContent == ""
    ? (clearBtn.innerHTML = emptyDisplayBtn)
    : (clearBtn.innerHTML = smthOnDisplayBtn);
  if (Calc.items.length > 0) {
...
function controlZero() {
  if (
    ...
    Calc.items[Calc.itemIndex] = Calc.items[Calc.itemIndex].substring(
      index,
      Calc.items[Calc.itemIndex].length
    );
